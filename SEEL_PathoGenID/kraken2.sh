...
for r1 in ./*R1*fastq.gz; do # solo itera sobre lecturas R1
   r2=${r1/_R1_/_R2_} # crea nombre para lecturas R2
...
for file in ${dir}/*species*.txt; do # para todo archivo que contenga la palabra short en el directorio KRAKEN2
...
# crear un solo archivo de resultados para todas las muestras en un directorio
for file in ${dir}/*short*.txt; do # para todo archivo que contenga la palabra short en el directorio KRAKEN2
   fname=$(basename $file | cut -d "_" -f "3") # guardar un nombre corto de archivo
   echo -e "\n$fname \n $(cat $file)" # mandar a standar output el nombre corto del archivo y su contenido
