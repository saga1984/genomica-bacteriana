...
for ensamble in ASSEMBLY/*.fa; do
   ename="$(basename ${ensamble} .fa)" # asignar nombre de ensamble (corto)
...
for file1 in $dir/*coverage*; do
   f1=$(basename ${file1} .fa) # obtener nombre corto de archivo, eliminando sufijo -cov.txt
   for file2 in $dir/*stats*; do
      f2=$(basename ${file2} .fa) # obtener nombre corto de archivo, eliminando sufijo -stats.txt
