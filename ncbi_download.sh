...
ncbi_download(){
  ...
  echo $(cat ftp_links_${genero}.txt | grep -c "Contig") "genomas nivel: Contig"
  ...
  echo $(cat ftp_links_${genero}.txt | grep -c "Complete") "genomas nivel: Complete"

  # recibir input del usuario numero y nivel de ensambles deseados
  echo -e "\nPor favor elige el nivel de calidad de ensamble que deseas"
  read -p "Elige entre: Contig (menor calidad), Scaffold, Chromosome o Complete (mayor calidad): " cal
