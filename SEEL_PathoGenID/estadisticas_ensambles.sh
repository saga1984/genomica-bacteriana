...
for ensamble in ASSEMBLY/*.fa; do
   ename="$(basename $ensamble | cut -d '-' -f '1')" # asignar nombre de ensamble (corto)
...
echo -e "ID,Contigs,Length,Largest_contig,N50,N90,N_count,Gaps,Depth,Coverage" > $dir/estadisticas_ensamble.csv
...
for file in $dir/*stats*; do
   ...
   # obtener profundidad
   depth=$(cat $file | sed -n '11p' | cut -d ' ' -f '3')
   # obtener cobertura
   coverage=$(cat $file |  sed -n '12p' | cut -d ' ' -f '3')
   # crea filas de archivo con datos obtenidos en las 3 filas anteriores
   echo -e "$fname,$contigs,$length,$largest,$N50,$N90,$n_count,$gaps,$depth,$coverage"
...
cat ${dir}/temp_total_stats.csv | sed '1d' | sed '1i ID,Contigs,Length,Largest_contig,N50,N90,N_count,Gaps,Depth,Coverage,R1,R2,PromR1R2' > $dir/estadisticas_totales.csv
