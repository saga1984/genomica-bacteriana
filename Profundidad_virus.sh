...
for ensamble in ASSEMBLY/*.fa; do
   ...
   for r1 in TRIMMING/*_1P.trim.fastq.gz; do # solo considerar read1 para el for loop
      read_name=$(basename $r1 | cut -d "/" -f "2" | cut -d "_" -f "1,2,3,4") # definir nombre del read
      r2=${r1/_1P./_2P.}
      echo -e "NOMBRES CONTROL:\t Ensamble = $ensamble_name\tRead = $read_name"
      # si el ensamble y el read no se llaman igual ignorarlos
      if [[ $ensamble_name != $read_name ]]; then
         continue
         echo -e "IF CONTROL:\tnombre_ensamble: $ensamble_name \tnombre_read: $read_name"

      # si el read y el ensamble se llaman igual entonces has todos los comandos siguientes
      else
         # controles
         echo -e "ELSE CONTROL:\tnombre_ensamble: $ensamble_name \tnombre_read: $read_name"
