...
BIGSdb_Lm_db_win="/home/senasica2/BIGSdb_Lm_db"
...
#for file in ${BIGSdb_Lm_db_win}/*.fa; do
...
# for loop para todos los ensambles dentro del directorio ASSEMBLY
   for ensamble in ASSEMBLY/*.fna; do
      # definir nombre del ensamble
      ensamble_name=$(basename ${ensamble} | cut -d '_' -f '1,2')
         ...
         blastn -query ${ensamble} -db ${BIGSdb_Lm_db_win}/${DB}.fa > ${ensamble_name}_${DB}.txt
...
cp -v ${BIGSdb_Lm_db_win}/profiles_Lm_serotype.txt $PWD/ # copia la base de datos que contiene el serotipo basado en perfil alelico al directorio actual
