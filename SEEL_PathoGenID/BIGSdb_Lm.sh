...
# for loop para todos los ensambles dentro del directorio ASSEMBLY
   for ensamble in ASSEMBLY/*.fa; do
      ...
      krakenGenus=$(awk '$4 == "G" && $6 == "Listeria" {print $6}' KRAKEN2/kraken_species_${ensamble_name}.txt | sed -n '1p')
      ...
      if [[ ${krakenGenus} == "Listeria"  && ${kmerfinderGenus} == "Listeria" ]]; then
...
awk 'BEGIN {FS=OFS="\t"} {for(i=1; i<=NF; i++) if($i ~ /^ *$/) $i = 0 }; {print $0}' tmp_profile > profile
