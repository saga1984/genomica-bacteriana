...
for r1 in *_R1*.fastq.gz; do # solo tomar una lectura (la 1)
   r2=${r1/_R1./_R2.} # sustituir en la posicion indicada _1 por _2 para convertir a lectura dos
   name=$(basename $r1 | cut -d "." -f "1") # cortar todo lo que va despues de "_" del nombre de r1
   $trimmomatic PE -threads $(nproc) $r1 $r2 \
   ...
   ILLUMINACLIP:${NexteraPE}:2:30:10 SLIDINGWINDOW:4:20 MINLEN:70 # parametros
done
